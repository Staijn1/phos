...
export class SegmentsComponent implements OnInit {
    ...
    updateSegment(): void {
        ...
        (this.segments)[this.segmentIndex].mode = <number>$('#modes').val();
        ...
        (this.segments)[this.segmentIndex].options |= $('#fade').val() ? <number>$('#fade').val() << 4 : 0;
        (this.segments)[this.segmentIndex].options |= $('#size').val() ? <number>$('#size').val() << 1 : 0;
    ...
    private changeRangeSlider(min: number, max: number) {
